...
// Replace the current SessionProvider from 'next-auth/react'
...
export function SessionDataProvider({
  ...
  useEffect(() => {
    const fetchSession = async () => {
      if (!initialSession) {
        ...
        const fetchedSession: Session | null =
          await fetchedSessionResponse.json()
  ...
  const sessionData = useMemo(() => {
    const authenticated = session ? 'authenticated' : 'unauthenticated'
